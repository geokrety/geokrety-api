...
class TestUserGeokretyOwned(GeokretyTestCase):
    """Test User owned operations"""
    ...
    def test_get_include_geokrety_owned_user1(self):
        """Check TestUserGeokretyOwned: GET user include GeoKrety owned - User1"""
        with app.test_request_context():
            self._blend_users()
            self._blend()
            url = '/v1/users/%d?include=geokrety-owned' % self.user1.id

            response = self._send_get(url, code=200)['included']
            self.assertEqual(len(response), 2)
            self._check_geokret(response[0], self.geokret2)
            self._check_geokret(response[1], self.geokret1)
    ...
    def test_get_include_geokrety_held_user3(self):
        """Check TestUserGeokretyOwned: GET user include GeoKrety owned - User3"""
        with app.test_request_context():
            self._blend_users()
            self._blend()
            url = '/v1/users/%d?include=geokrety-owned' % self.user3.id
