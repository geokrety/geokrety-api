...
class TestGeokretCreate(BaseTestCase):
    ...
    @request_context
    def test_geokret_create_geokrety_type_missing(self):
        payload = GeokretyPayload()
    ...
    @request_context
    def test_geokret_create_field_name_accept_unicode(self, name, result):
        payload = GeokretyPayload()
        payload.set_name(name)
        response = self.send_post(payload, user=self.user_1, code=201)
    ...
    @request_context
    def test_geokret_create_field_name_doesnt_accept_html(self, name, result):
        payload = GeokretyPayload()
        payload.set_name(name)
        response = self.send_post(payload, user=self.user_1, code=201)
        response.assertHasAttribute('name', result)
    ...
    @parameterized.expand(UTF8_TEST_CASES)
    @request_context
    def test_geokret_create_field_description_accept_unicode(self, description, result):
        payload = GeokretyPayload()
    ...
    @request_context
    def test_geokret_create_field_description_accept_html_subset(self, description, result):
        payload = GeokretyPayload()
        payload.set_description(description)
        response = self.send_post(payload, user=self.user_1, code=201)
        response.assertHasAttribute('description', result)
