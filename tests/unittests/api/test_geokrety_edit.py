...
class TestGeokretyEdit(BaseTestCase):
    ...
    @request_context
    def test_geokret_patch_as(self, input, expected):
    ...
    @request_context
    def test_geokret_patch_when_user_has_touched(self, input):
        payload = GeokretyPayload()
        new_name = 'some other name'
        payload._set_attribute('name', new_name)
        ...
        self.blend_move(geokret=geokret, author=self.user_2,
                        move_type_id=MOVE_TYPE_GRABBED,
                        moved_on_datetime="2018-09-22T18:18:57")
        response = self.send_patch(geokret.id, payload, user=self.user_1, code=403)
    ...
    def test_geokret_patch_relationships_owner_as_authenticated(self):
        ...
        payload._set_relationships('owner', 'user', self.user_2.id)
        geokret = self.blend_geokret(owner=self.user_1)
        response = self.send_patch(geokret.id, payload, user=self.user_2, code=403)
    ...
    @request_context
    def test_geokret_patch_relationships_holder_as(self, input):
        payload = GeokretyPayload()
    ...
    @request_context
    def test_geokret_patch_relationships_moves_as(self, input):
        payload = GeokretyPayload()
