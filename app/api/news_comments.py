...
class NewsCommentList(ResourceList):
        ...
        if view_kwargs.get('news_id') is not None:
            try:
                self.session.query(News).filter_by(
                    id=view_kwargs['news_id']).one()
            except NoResultFound:
                raise ObjectNotFound(
                    {'parameter': 'news_id'}, "News: {} not found".format(view_kwargs['news_id']))
            else:
    ...
    def before_create_object(self, data, view_kwargs):
        ...
        if view_kwargs.get('news_id') is not None:
            try:
                news = self.session.query(News).filter_by(
                    id=view_kwargs['news_id']).one()
            except NoResultFound:
                raise ObjectNotFound(
                    {'parameter': 'news_id'}, "News: {} not found".format(view_kwargs['news_id']))
...
class NewsCommentDetail(ResourceDetail):
    decorators = (
        api.has_permission('is_user_itself', methods="DELETE,PATCH",
